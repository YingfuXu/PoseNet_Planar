...
class ICSTNPyramid(nn.Module): # build Inverse Compositional STN
    ...
    def forward(self,batch_img1,batch_img2,batch_homo8):
        ...
        ## block 4
        if self.trace_model: # for cpp
            batch_img2_warped_4 = self.img_warper.transformImageTrans_Single(batch_img2_deRot,batch_nn_trans_sum,zeroRotMtrx,batch_planeNormVecImg1)
        else:
            batch_img2_warped_4 = self.img_warper.transformImageTrans(batch_img2_deRot,batch_nn_trans_sum,zeroRotMtrx,batch_planeNormVecImg1)
        ...
        if self.training:
            ...
            if self.self_sup:
                batch_img2_warped_5,valid_pixel_mask = self.img_warper.transformImageTrans(batch_img2,batch_homo8[:, 5:],batch_rotMtrx,batch_planeNormVecImg1,move_out_mask=True)
